...
responses = requests.post(url = url, data = json.dumps(body), headers =
    headers)
return responses
def get_info1():
    url = "http://hslhapp.hhws168.com/common/v1/jsonDownload/"
...
str1 = ''.join(list1)
list2 = json.loads(str1)# for i in range(len(list2)): #print(list2[i])
dict_result = {}
list3 = list()
for _ in list2:
    for k, v in _.items():
...
or k == 'getTime'
or k == 'firstGetTime':
    dict_result[k] = v# print(dict_result)
...
for i in range(len(last_list)):
    print(last_list[i])
if len(list_result) != len(last_list): #数据数量不一致， 直接推送
for i in range(len(list_result)): #WechatMessagePush(appid, screct,
    template_id).send_wechat_temple_msg(list_result[i])
...
for i in range(len(push_list)):
    print(push_list[i])
if len(push_list):
    for i in range(len(push_list)): #WechatMessagePush(appid, screct,
        template_id).send_wechat_temple_msg(push_list[i])
push_report(push_list[i], web_hook)
else :
    print("没有数据更新,不推送")
...
ChatBot = requests.post(url = web_hook, data = send_data, headers =
    header)
opener = ChatBot.json()
if opener["errmsg"] == "ok":
...
webhook =
    "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=f38433df-3a2a-46d3-bd45-8d31bf8adc94"
webhook1 =
    "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=8eed9b02-38de-4d20-939c-278214128005"
webhook2 =
    "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=9b6a9289-c683-4f42-9a77-04c4384cf19e"
webhook3 =
    "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=0810ff88-06be-46dc-9d67-07e1d894ad95"
